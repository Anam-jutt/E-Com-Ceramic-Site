    ...
    <nav className="bg-[#025048] text-gray-200 shadow-md sticky top-0 z-50">
      <div className="container mx-auto px-4 py-3 flex justify-between items-center">
        ...
        <div className={`${windowWidth >= 768 ? "flex" : "hidden"} gap-4 items-center`}>
          ...
          {token && role === "buyer" && (
            <Link to="/cart" className="text-2xl flex gap-1 hover:scale-110 transition duration-300 relative">
              <FaCartPlus /><sup className="text-base">{totalItems}</sup>
            </Link>
          )}
        </div>
      ...
      {menuOpen && windowWidth < 768 && (
        <div className="px-4 pb-4 space-y-3">
          <Link to="/" className="block hover:text-rose-400" onClick={toggleMenu}>Home</Link>
          <Link to="/shop" className="block hover:text-rose-400" onClick={toggleMenu}>Shop</Link>
          <Link to="/about" className="block hover:text-rose-400" onClick={toggleMenu}>About</Link>
          <Link to="/contact" className="block hover:text-rose-400" onClick={toggleMenu}>Contact</Link>

          {token && role === "seller" && (
            <>
              <Link to="/admin/homes" className="hover:text-rose-400 focus:text-rose-300 duration-500">My Listing</Link>
              <Link to="/admin/products/new" className="block hover:text-rose-400 pt-3" onClick={toggleMenu}>Add Product</Link>
            </>
          )}

          {token && role === "buyer" && (
            <>
              <Link to="/cart" className="block hover:text-rose-400" onClick={toggleMenu}>Cart</Link>
            </>
          )}

          {!token ? (
            <>
              <Link to="/login" className="block hover:text-rose-400" onClick={toggleMenu}>Login</Link>
              <Link to="/signup" className="block hover:text-rose-400" onClick={toggleMenu}>Sign Up</Link>
            </>
          ) : (
            <button onClick={() => { toggleMenu(); logout();}} className="block cursor-pointer text-left hover:text-rose-400 w-full">
              Logout
            </button>
          )}
        </div>
